...
// 模板参数 T 代表类型
    template<class T>
    class vector {
        static_assert(!std::is_same<bool, T>::value, "myVector<bool> is abandoned in mystl");
    public:
        // myVector 的嵌套型别定义
    ...
    public:
        ...
        reference at(size_type n) {
            THROW_OUT_OF_RANGE_IF(!(n < size()), "myVector<T>::at() subscript out of range");
            return (*this)[n];
        }
    ...
    void vector<T>::reserve(size_type n) {
        if (capacity() < n) {
            THROW_LENGTH_ERROR_IF(n > max_size(),
                                  "n can not larger than max_size() in myVector<T>::reserve(n)");
